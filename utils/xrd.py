...
def read_txt(filename) -> pd.DataFrame:
    ...
    with open(filename, "r") as f:
        ...
        if ";RAW4.00" in first_line:
            data_start_line = [i for i, line in enumerate(f) if line.rstrip() == "[Data]"][0] + 3 # skip 3 lines past [Data]
            data = pd.read_csv(filename, skiprows=data_start_line, sep=",", names=["TwoTheta", "Intensity", "x"])
            data = data.drop(columns=["x"])
        
        elif "[°2Th.]" in first_line:
            data = pd.read_csv(filename, sep="\t", skiprows=1, names=["TwoTheta", "Intensity","x", "xx", "xxx", "xxxx", "xxxxx"])
